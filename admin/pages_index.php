...
if (isset($_POST['login'])) {
  ...
  $stmt->store_result(); //store result to get the number of rows

  if ($stmt->num_rows > 0) { //if login successful
    $stmt->bind_result($email, $password, $admin_id); //bind result
    $stmt->fetch(); //fetch result

    // Insert login data
    $role = 'Admin'; // Assuming the role is always 'admin' for admins
    ...
    $insert_stmt->bind_param("ssss", $admin_id, $email, $ldate, $role);
    $insert_stmt->execute();
    ...
    if ($insert_stmt->affected_rows > 0) {
      ...
      $err = "Error inserting login data.";
    }
  } else {
