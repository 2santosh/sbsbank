...
$query_check_account = "SELECT COUNT(*) AS count FROM iB_bankAccounts WHERE client_id = ?";
$stmt_check_account = $mysqli->prepare($query_check_account);
$stmt_check_account->bind_param('s', $client_id);
$stmt_check_account->execute();
$stmt_check_account->store_result();
$stmt_check_account->bind_result($account_count);
$stmt_check_account->fetch();

if ($account_count > 0) {
    // Client already has an account
    $err = "Client already has an account";
} else {
    // Proceed with opening a new account
    if (isset($_POST['open_account'])) {
        ...
        // Bind parameters
        $stmt->bind_param('sssssssssssss', $acc_name, $account_number, $acc_type, $acc_rates, $acc_status, $acc_amount, $client_id, $client_name, $client_national_id, $client_phone, $client_number, $client_email, $client_adr);
        $stmt->execute();
