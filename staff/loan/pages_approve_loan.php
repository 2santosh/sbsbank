    ...
    if ($loan_data) {
        if ($_SERVER["REQUEST_METHOD"] == "POST") {
            ...
            if (isset($_POST['action'])) {
                $action = $_POST['action'];
                if ($action === "Approve" || $action === "Reject" || $action === "Closed") { // Check for "Approve" or "Reject" action
                    // Update loan status
        ...
        $err = "Loan data not found.";
    }
} else {
    // Handle the case where loan_id is not set
    $err = "Loan ID not provided.";
}
?>
...
<body class="hold-transition sidebar-mini layout-fixed layout-navbar-fixed">
                    ...
                    <div class="row mb-2">
                        ...
                        <div class="col-sm-6">
                            <ol class="breadcrumb float-sm-right">
                                ...
                                <li class="breadcrumb-item"><a href="manage_loans.php">Manage Loans</a></li>
                        ...
                        <div class="col-md-6">
                            <div class="card">
                                <div class="card-body">
                                    ...
                                    <!-- View Document buttons for loan and deposit documents -->
    ...
    <div style="display:none;" id="loan_document_viewer">
        <embed src="<?php echo $loan_data['loan_document']; ?>" width="100%" height="600px" />
    </div>
<?php endif; ?>
...
<?php if (!empty($loan_data['deposit_document'])) : ?>
    ...
    <div style="display:none;" id="deposit_document_viewer">
        <embed src="<?php echo $loan_data['deposit_document']; ?>" width="100%" height="600px" />
    </div>
<?php endif; ?>
                                    </p>
                                </div>
    ...
    <script>
    // Function to toggle visibility of document viewer
    function toggleDocumentViewer(documentType) {
        if (documentType === 'loan') {
            var loanDocumentViewer = document.getElementById('loan_document_viewer');
            if (loanDocumentViewer.style.display === 'none') {
                loanDocumentViewer.style.display = 'block';
                // Hide deposit document viewer if open
                document.getElementById('deposit_document_viewer').style.display = 'none';
            } else {
                loanDocumentViewer.style.display = 'none';
            }
        } else if (documentType === 'deposit') {
            var depositDocumentViewer = document.getElementById('deposit_document_viewer');
            if (depositDocumentViewer.style.display === 'none') {
                depositDocumentViewer.style.display = 'block';
                // Hide loan document viewer if open
                document.getElementById('loan_document_viewer').style.display = 'none';
            } else {
                depositDocumentViewer.style.display = 'none';
            }
        }
    }
</script>
