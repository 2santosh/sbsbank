...
if (isset($_POST['reset_password'])) {
    ...
    $checkData = mysqli_query($mysqli, "SELECT * FROM `IB_staff` WHERE `email` = '$email' AND `name` = '$name' AND `phone` = '$phone'") or exit(mysqli_error($mysqli));
    if (mysqli_num_rows($checkData) > 0) {
        // Email, name, and phone match, proceed with password reset

        // Check if the email already exists in the database
        $checkEmail = mysqli_query($mysqli, "SELECT `email`, `position` FROM `ib_password_resets` WHERE `email` = '" . $_POST['email'] . "'") or exit(mysqli_error($mysqli));
        if (mysqli_num_rows($checkEmail) > 0) {
            // Email already exists, retrieve position and insert new reset request
            ...
            // Insert new password reset request with status 'pending' at the retrieved position
            $query = "INSERT INTO ib_password_resets (email, reset_token, status, position) 
                      VALUES (?, ?, 'pending', ?)";
            $stmt = $mysqli->prepare($query);
            // Bind parameters
            $stmt->bind_param('ssi', $email, $reset_token, $position);
            $stmt->execute();
            ...
            // Email does not exist, proceed with inserting new reset request
            // Generate a reset token
            $reset_token = bin2hex(random_bytes(16));
        ...
        $errorMsg = "Email, name, and phone do not match with existing data.";
    }
}
?>
