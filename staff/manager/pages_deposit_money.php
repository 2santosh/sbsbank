...
if (isset($_POST['deposit'])) {
    ...
    if (!is_numeric($transaction_amt) || $transaction_amt < 100 || $transaction_amt > 1000000) {
        $err = "Invalid amount. Please enter a positive number less than 1000000 or equal to 100.";
    } else {
        ...
        // Initialize acc_amount if not initialized
        if (!isset($acc_amount)) {
            $acc_amount = 0;
        }
        // Increment acc_amount
        $acc_amount += $transaction_amt;

        // Notification details
        $notification_details = "$client_name has deposited $$transaction_amt to bank account $account_number";

        // Insertion queries
        $query = "INSERT INTO iB_Transactions (tr_code, account_id, acc_name, account_number, acc_type, acc_amount, tr_type, tr_status, client_id, client_name, client_national_id, transaction_amt, client_phone) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?)";
        $notification = "INSERT INTO  iB_notifications (notification_details) VALUES (?)";

        // Update account balance query for iB_bankaccounts table
        $update_balance_query_account = "UPDATE iB_bankaccounts SET acc_amount = acc_amount + ? WHERE account_id = ?";
        // Update account balance query for iB_Transactions table
        $update_balance_query_transaction = "UPDATE iB_Transactions SET acc_amount = ? WHERE account_id = ?";

        $stmt = $mysqli->prepare($query);
        $notification_stmt = $mysqli->prepare($notification);
        $update_balance_stmt_account = $mysqli->prepare($update_balance_query_account);
        $update_balance_stmt_transaction = $mysqli->prepare($update_balance_query_transaction);

        // Bind parameters
        $rc = $notification_stmt->bind_param('s', $notification_details);
        $rc = $stmt->bind_param('sssssssssssss', $tr_code, $account_id, $acc_name, $account_number, $acc_type, $acc_amount, $tr_type, $tr_status, $client_id, $client_name, $client_national_id, $transaction_amt, $client_phone);
        $rc = $update_balance_stmt_account->bind_param('ss', $transaction_amt, $account_id);
        $rc = $update_balance_stmt_transaction->bind_param('ss', $acc_amount, $account_id);

        // Execute queries
        $stmt->execute();
        $notification_stmt->execute();
        $update_balance_stmt_account->execute();
        $update_balance_stmt_transaction->execute();

        // Check if insertion and balance updates were successful
        if ($stmt && $notification_stmt && $update_balance_stmt_account && $update_balance_stmt_transaction) {
