...
$ret = "SELECT account_id FROM iB_bankAccounts WHERE client_id = ?";
$stmt = $mysqli->prepare($ret);
$stmt->bind_param('i', $client_id);
$stmt->execute();
$res = $stmt->get_result();

// Fetch the result and extract the account_id
$row = $res->fetch_assoc();
...
if ($row) {
    ...
    sleep(2);
    $err = "No account found for the given client ID.";
    header("Location: pages_balance_enquiries.php?error=" . urlencode($err));
